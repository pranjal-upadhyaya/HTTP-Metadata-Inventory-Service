...
router = APIRouter(prefix="/metadata_inventory", tags=["Metadata Inventory"])
...
async def scrape_metadata(
    ...
    try:
        service_response: ScrapeMetadataResponse = await service.scrape_metadata(
            request=request
        )
    except httpx.RequestError as e:
        raise URLFetchError(url=str(request.url), reason=str(e)) from e
    except DuplicateKeyError as e:
        raise DuplicateURLError(url=str(request.url)) from e
...
async def fetch_metadata(
    url: str, service: HTTPMetadataInventoryService = Depends(get_service)
) -> JSONResponse:
    try:
    ...
    service_response: FetchMetadataResponse = await service.fetch_metadata(
        request=request
    )

    if service_response.metadata_available:
        return JSONResponse(data=service_response, message=None)

    return JSONResponse(
        data=None, message="Url metadata request logged", status_code=202
    )
